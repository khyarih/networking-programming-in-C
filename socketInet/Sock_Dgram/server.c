...
int main()
{
    ...
    if (_socket < 0)
        showError("socket", "");
    //hostent
    // hostent = gethostbyname("DESKTOP-OK6ONBF");
    //hostent = gethostbyaddr(ip,length,type);

    //bind
    sockaddr_in_s.sin_family = AF_INET;
    sockaddr_in_s.sin_port = htons(6000);
    sockaddr_in_s.sin_addr.s_addr = inet_addr("127.0.0.1");
    // sockaddr_in_s.sin_zero =
    _bind = bind(_socket, (struct sockaddr *)&sockaddr_in_s, sizeof(sockaddr_in_s));
    if (_bind < 0)
        showError("bind", "");
    ...
    sockaddr_in_c.sin_port = htons(5000);
    sockaddr_in_c.sin_addr.s_addr = inet_addr("127.0.0.1");

    _sendto = sendto(_socket, msg, sizeof(msg), 0, (struct sockaddr *)&sockaddr_in_c, size);
    if (_sendto < 0)
        showError("sendto", "");
    return 0;
}